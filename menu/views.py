...
def index(request):
    if 'search_filter' in request.GET:
        ...
        # pizza = list(Pizza.objects.filter(name__icontains=search_filter).values()
...
def get_pizza_by_id(request, id):
    return render(request, pizza, 'menu/pizza_details.html', {
        'pizza': get_object_or_404(Cart, pk=id)
    })
...
def update_pizza(request, id):
    ...
    return render(request, 'pizza/update_pizza.html', {
        'form': form,
        'id': id
    })
